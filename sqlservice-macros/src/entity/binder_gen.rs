...
pub fn generate_to_params(info: &StructInfo) -> TokenStream {
    ...
    let param_entries: Vec<TokenStream> = active_non_ignored
        ...
        .map(|f| {
            ...
            quote! {
                sqlservice::params::OdbcParam::new(
                    #param_name,
                    sqlservice::params::ParamValue::from(self.#rust_ident.clone()),
                )
            }
        })
        .collect();
    ...
    quote! {
        fn to_params(&self) -> Vec<sqlservice::params::OdbcParam> {
            vec![
                #(#param_entries),*
            ]
        }
...
pub fn generate_pk_value(info: &StructInfo) -> TokenStream {
    ...
    if let Some(pk) = pk {
        ...
        quote! {
            fn pk_value(&self) -> sqlservice::params::PkValue {
                sqlservice::params::PkValue::from(self.#rust_ident.clone())
            }
        }
    } else {
