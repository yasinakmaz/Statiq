...
impl GeneratedSql {
    pub fn from_struct(info: &StructInfo) -> Self {
        ...
        let select_by_pk_sql = format!(
            "SELECT {select_cols} FROM {fq} WHERE {pk_col} = @{pk_rust}"
        );
        ...
        // DELETE
        let delete_sql = format!("DELETE FROM {fq} WHERE {pk_col} = @{pk_rust}");
        ...
        let exists_sql = format!("SELECT CAST(1 AS BIT) FROM {fq} WHERE {pk_col} = @{pk_rust}");
